...
export default function EventsPage() {
    ...
    const handleDeleteEvent = async (id) => {
        try {
            const response = await fetch(`/api/users/events?id=${id}`, {
                method: 'DELETE',
            });
            const data = await response.json();
            if (data.success) {
        ...
        <div className="container mx-auto p-4">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Create and manage events with comprehensive details</CardDescription>
                </CardHeader>
                <CardContent>
                    <Dialog
                        open={isDialogOpen}
                        ...
                        <TableBody>
                            {events.map((event) => (
                                <TableRow key={event._id}>
                                    ...
                                    <TableCell>{event.speaker?.name || 'N/A'}</TableCell>
                                    <TableCell>
                                        {event.date
